import { fromPairs, map, mapValues, omit } from 'lodash';
...
const transformFile = (file_path: string) =>
  fs.readFile(file_path, 'utf8', (_err, data) => {
    const modules: { [moduleCode: string]: { [lessonType: string]: RawLesson[] } } =
      JSON.parse(data);
    const modifiedModules = mapValues(modules, (module) =>
      mapValues(module, (lessonsWithLessonType) =>
        fromPairs(
          map(lessonsWithLessonType, (lesson) => [
            serializeLessonDetails(lesson),
            omit(lesson, 'serializedLessonDetails'),
          ]),
        ),
      ),
    );
    const stringified = JSON.stringify(modifiedModules, null, 2);
    const formattedStringified1 = stringified.replaceAll('[\n            ', '[');
    const formattedStringified2 = formattedStringified1.replaceAll('\n            ', ' ');
    const formattedStringified3 = formattedStringified2.replaceAll('\n          ]', ']');
    fs.writeFile(file_path, formattedStringified3, () => {});
  });
