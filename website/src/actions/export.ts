...
import { hydrateSemTimetableWithLessons, migrateTaModulesConfig } from 'utils/timetables';
  ...
  return (_dispatch: Dispatch, getState: GetState) => {
    Promise.all([
      ...
      .then(([ical, icalUtils]) => {
        ...
        const taModuleConfig = state.timetables.ta[semester] ?? [];
        const taModules = isArray(taModuleConfig)
          ? taModuleConfig
          : migrateTaModulesConfig(taModuleConfig);
