 ...
 * @returns map of lesson ids, not lessons
 */
...
/**
 * Used to recover from the config of a lesson type with no lessons
 ...
 * Note: the current implementation generates a config containing lessons belonging to the first classNo in the provided lessons
 */
export function getRecoverySerializedLessonId(
 ...
 * @param lessonIdsMap {@link LessonIdsMap|Lesson indices mapping} of the module
 * @param timetableLessonIds lessons currently in lesson config
...
export function getClosestLessonConfig(
  ...
  timetableLessonIds: ModuleLessonConfig,
): ModuleLessonConfig {
  return reduce(
    lessonMap,
    (accumulatedModuleLessonConfig, lessonTypeLessons, lessonType) => {
      const configLessonTypeLessonIds: LessonId[] = get(timetableLessonIds, lessonType);
      const closestClassNo = getClosestClassNo(lessonTypeLessons, configLessonTypeLessonIds);
