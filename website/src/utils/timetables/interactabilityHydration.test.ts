...
import { LessonId, LessonType, ModuleCode, RawLesson } from 'types/modules';
...
describe('getInteractableLessons', () => {
  ...
  describe('hydrating modules when there is an active lesson from a non-ta module', () => {
    const activeLessonId = 'F01|FRI|1400|1700|S12-0402|3_5_7_9_11';
    ...
    const readOnly = false;
    const activeLesson = get(timetableLessons, [PC1222.moduleCode, 'Laboratory', activeLessonId]);
  ...
  describe('hydrating modules when there is an active lesson from a ta module', () => {
    const activeLessonId = '1|TUE|1400|1600|AS6-0421|3_4_5_6_7_8_9_10_11_12_13';
    ...
    const readOnly = false;
    const activeLesson = get(timetableLessons, [CS4243.moduleCode, 'Laboratory', activeLessonId]);
    ...
    const hydratedOtherModuleLessons = get(hydratedLessons, PC1222.moduleCode);

    test('lessons that are not from the same module as the active lesson cannot be added', () => {
      expect(
        some(flatMap(hydratedOtherModuleLessons), (lesson) => lesson.canBeAddedToLessonConfig),
      ).toBe(false);
    });

    test('timetable lessons from other modules are visible', () => {
      const otherModuleLessons = mapValues(get(lessonsMap, PC1222.moduleCode), (lessons) =>
        keys(lessons),
      );
  ...
  describe('hydrating ta module containing multiple lessons with the same classNo', () => {
    const timetableLessonIds = ['SL1|MON|1600|1800|LT27|1_2_3_4_5_6_7_8_9_10_11_12_13'];
    ...
    describe('when a lesson is active', () => {
      ...
      test('lesson already in timetable are already in the lesson config', () => {
        const hydratedTimetableLessons = filter(hydratedLessonsArray, (lesson) =>
          timetableLessonIds.includes(serializeLessonDetails(lesson)),
        );
