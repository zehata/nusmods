...
describe(validateModuleLessons, () => {
  ...
  describe('validate non ta module lessons', () => {
    ...
    test('should leave valid lessons untouched', async () => {
      await expect(validateModuleLessons(semester, lessons, CS1010S, false)).resolves.toStrictEqual(
        {
    ...
    test('should convert serializedLessonDetails lessonId to classNo lessonId', async () => {
      await expect(
    ...
    test('should recover invalid lessonId based on classNo', async () => {
      await expect(
        validateModuleLessons(
          semester,
          {
            ...lessons,
            Recitation: ['2|WED|1300|1400|S14-0619|1_2_3_4_5_6_7_8_9_10_11_12_13'],
          },
          CS1010S,
          false,
        ),
    ...
    test('should remove lesson types which do not exist in module', async () => {
      await expect(
        validateModuleLessons(
          semester,
          {
      ...
      ).resolves.toStrictEqual({ validatedLessonConfig: lessons, valid: false });
    });

    test('should add lessons for when they are missing', async () => {
      await expect(
        validateModuleLessons(
          semester,
          {
  ...
  describe('validate ta module lessons', () => {
    ...
    test('should leave valid config untouched', async () => {
      await expect(validateModuleLessons(semester, lessons, CS4243, true)).resolves.toStrictEqual({
        validatedLessonConfig: lessons,
        valid: true,
      });
    });

    test('should convert classNo lessonId to serializedLessonDetails', async () => {
      await expect(
        validateModuleLessons(
          semester,
          {
    ...
    test('should replace lessons that have invalid class no', async () => {
      await expect(
        validateModuleLessons(
          semester,
          {
