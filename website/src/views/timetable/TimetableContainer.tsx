...
import { deserializeSemTimetableConfig } from 'utils/timetables';
...
import { getImportedModuleCodes } from 'utils/timetables/shareLinks';
...
export const TimetableContainerComponent: FC = () => {
  ...
  useEffect(() => {
    ...
    const {
      ...
      ta: taModules,
    } = deserializeSemTimetableConfig(location.search, modules, semester);

    setImportedTimetable(semTimetableConfig);
    setImportedHidden(hiddenModules);
    setImportedTa(taModules);
  }, [modules, semester, params.action, isLoading, location.search]);
