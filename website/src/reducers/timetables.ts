...
import { SerializedLessonDetails, ModuleCode } from 'types/modules';
...
function moduleLessonConfig(
  ...
  switch (action.type) {
    case CHANGE_LESSON: {
      const { serializedLessonDetails, lessonType } = action.payload;
      if (!(serializedLessonDetails && lessonType)) return state;
      return {
        ...state,
    ...
    case ADD_LESSON: {
      ...
      return {
        ...state,
        [lessonType]: uniq([...serializedLessonDetails, ...state[lessonType]]),
      };
    }
    case REMOVE_LESSON: {
