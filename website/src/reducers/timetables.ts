...
function moduleLessonConfig(
  ...
  switch (action.type) {
    ...
    case ADD_LESSON: {
      ...
      return {
        ...state,
        [lessonType]: uniq([...lessonIds, ...state[lessonType]]),
      };
    }
    case REMOVE_LESSON: {
      const { lessonIds: lessonIdsToExclude, lessonType } = action.payload;
      return {
        ...state,
        [lessonType]: [
          ...state[lessonType].filter(
            (lessonId: LessonId) => !lessonIdsToExclude.includes(lessonId),
          ),
        ],
      };
    }
