...
func CreateConfig(assignments map[string]string, lessonToSlots map[string][][]models.ModuleSlot) map[string]map[string][]models.LessonIndex {
	...
	for lessonKey, lessonGroup := range assignments {
		...
		for _, lessonsInLessonGroup := range lessonToSlots[lessonKey] {
			if lessonsInLessonGroup[0].LessonGroup == lessonGroup {
				for _, lesson := range lessonsInLessonGroup {
